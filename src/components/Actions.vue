<template>
  <el-col class="actions-container-top">
    ...
    />
    <el-select v-model="routeStore.profile" style="width: 250px" @change="handleChangeProfile">
      ...
      <el-option v-for="item in profileOptions" :key="item" :label="item" :value="item">
        <div class="select-label-wrapper">
          <el-icon>
  ...
  <el-col class="actions-container-left-bottom">
    <div>
      <a href="https://github.com/ValentinM27/opendata-route" rel="noreferrer" target="_blank">
        GitHub
      </a>
...
<script setup lang="ts">
  ...
  const moveToPosition = (selectedPosition: AutoComplete) => {
    const pointToFocus = new OlPoint(fromLonLat(selectedPosition.feature.geometry.coordinates));
  ...
  const handleAltimetryData = (altimetryLine: Elevation[]) => {
    ...
    altimetryLine.forEach((altimetryPoint) => {
      ...
      const pointDistance = routeStore.samplePointsDistance.get(altimetryPointKey);
